...
// button elements (clickable DIVs) that control the classes 
// to collect the respective video frames during the training
let rock_button;
let paper_button;
let scissors_button;
...
function setup() {
  ...
  //create DIVs with the respective CSS-class containing the button elements
...
//play a round vs. AI
function goPlay() {
  if (acc_dict[rock_button.id] < 100 || acc_dict[paper.id] < 100 || acc_dict[scissors.id] < 100) {
...
function findWinner() {
  if (win_matrix[test_label][ai_move] > 0) winner.html("Player's " + test_label + " beats AI's " + ai_move + '. you win!');
  else if (win_matrix[test_label][ai_move] < 0) winner.html("AI's " + ai_move + " beats Player's " + test_label + '. you lose!');
  else (winner.html('Player and AI both threw ' + test_label + '. It\'s a tie!'));
}
...
function draw() {
  ...
  if (playing) {
    ...
    countdown();
  } else if (acc_dict[rock_button.id] == 100 && acc_dict[paper.id] == 100 && acc_dict[scissors.id] == 100) {
